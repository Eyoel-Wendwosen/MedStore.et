...
class ProductDetail extends Component {
	...
	async componentDidMount() {
		...
		let productN = (await axios.get(`${LOCAL_BASE_URL}${API_URL}/product/${product._id}`)).data.data
		productN.photo_urls = productN.photo_urls.map(url => {
			return {
				src: url,
				altText: '',
				caption: '',
	...
	handleRequestSubmit(e) {
		...
		// let request = { product: this.state.product._id, ...formValues }
		let request = {
							...
							<Col lg="4" className="mb-lg-auto">
								<div className="rounded shadow-lg overflow-hidden ">
									<UncontrolledCarousel
										...
										controls={true}
										indicators={true}
										items={this.state.product ? this.state.product.photo_urls : ''}
									/>
									<CardImg
										className="p-1"
										alt="Image not found"
							...
							<Col className="order-2 product-characterstics bg-grey p-2 rounded shadow mr-5">
								...
								{product
									? product.characteristics.map(char => (
											<Row className="ml-2">
												<Col sm="4">
													<small className="text-capitalize  font-weight-bold">{char.name}:</small>
												</Col>
												<Col sm="8">
													<p>{char.value}</p>
												</Col>
								...
								<Col lg="" className="p-0">
									<Card className="bg-gradient-secondary shadow">
										<CardBody className="p-lg-5">
											<h4 className="mb-1">
												Want to know more about the {this.state.product.name}
											</h4>
