        ...
        <ul className="list-group">
            {categories.map((category) => (
                <li
                    ...
                    {category.subCategories.length !== 0 && (
                        <span
                            id={`Category_${category.name.split(" ")[0]}`}
                            class="material-icons rigth-align">
                            expand_more
                        </span>
                    )}
                    {category.subCategories.length !== 0 &&
                            ...
                            <UncontrolledCollapse toggler={`#Category_${category.name.split(" ")[0]}`}>
                                <ul className="list-group">
                                    {category.subCategories.map(sub_cat => (
                                        <Link
                                            to={`${path}/subCategory/${sub_cat._id}`}
                                        >
...
const Categories = (props, match) => {
    ...
    useEffect(() => {
        axios.get(`${LOCAL_BASE_URL}${API_URL}/category`).then(res => {
    ...
    return (
        <>
            <div>
                ...
                <span onClick={() => collapseCategoriesView()} className="display-4 pt-1 material-icons"> more_vert</span>
