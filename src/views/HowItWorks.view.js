				...
				<section className="section bg-gradient-teal section-shaped">
					...
					<div className="content-center pt-2 text-white">
						<Container>
							...
							<p className="px-8">
								<span className="font-weight-bold">{this.state.title}</span>
							</p>
						</Container>
					</div>
						...
						this.state.content.map((c, index) => (
							<Row className="pt-5">
								<Col className={`content-center pr-md-5 order-md-${index % 2 ? 2 : 1}`} md="6">
									<span className=" text-teal icon-l material-icons-outlined">{c.icon}</span>
								</Col>
								<Col className={`order-md-${index % 2 ? 1 : 2}`}>
									<h2 className="content-center text-uppercase">
										<span>{c.title}</span>
									</h2>
									<p>{c.description}</p>
								</Col>
